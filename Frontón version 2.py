...
canvas.pack()
bat=canvas.create_rectangle(0, 0, 40, 10, fill="white")
ball=canvas.create_oval(20, 0, 30, 10, fill="yellow")
windowOpen=True
def main_loop():
...
def check_game_over():
    ...
    if ballTop>canvasHeight:
        playAgain=tkinter.messagebox.askyesno(message="¿Quieres seguir jugando?")
        if playAgain==True:
            reset()
        else:
            close()
